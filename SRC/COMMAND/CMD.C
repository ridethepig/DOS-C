...
/* $Logfile:   C:/usr/patv/dos-c/src/command/cmd.c_v  $ */

/* $Log:   C:/usr/patv/dos-c/src/command/cmd.c_v  $ 
 * 
 *    Rev 1.7   06 Dec 1998  8:42:30   patv
 * Fixed a parse error.
 * 
 *    Rev 1.6   23 Feb 1998  5:41:58   patv
 * Eliminated DEBUG switch around setting of psp_parent
 * 
 *    Rev 1.5   22 Feb 1998  9:44:18   patv
...
static BYTE *RcsId = "$Header:   C:/usr/patv/dos-c/src/command/cmd.c_v   1.7   06 Dec 1998  8:42:30   patv  $";
#endif
...
VOID main()
{
	...
	*tail = '\0';
	...
	if(*esize != '\0')
	{
		...
		bEnvAllocated = EnvAlloc(size);
		EnvSize = size;
	}
	else
		bEnvAllocated = EnvAlloc(EnvSize);
